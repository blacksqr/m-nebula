...
/**
	...
	@brief Implemented reference to moveable target object.
    ...
    nHardRef has 3 states:<br>
        1) name valid, pointer invalid<br>
        2) pointer valid, name invalid<br>
        3) name invalid, pointer invalid<br>
...
public:
    ...
    nHardRef(nKernelServer* kServer, nRoot* ownerObject);
    /// the destructor
    ~nHardRef();
    
    /// defered initialization, call if default constructor used 
    void initialize(nKernelServer* kServer, nRoot* ownerObject);
...
*/
template<class TYPE>
nHardRef<TYPE>::nHardRef(nKernelServer* kernelServer, nRoot* ownerObject)
: ks(kernelServer),
  nRef<TYPE>(ownerObject),
  strNode((void*) this)
{
    // empty
}
...
template<class TYPE>
void
nHardRef<TYPE>::initialize(nKernelServer* kernelServer, nRoot* ownerObject)
{
    this->ks = kernelServer;
    nRef<TYPE>::initialize(ownerObject);
}
...
nHardRef<TYPE>::set(const char* objName)
{
    ...
    if (objName)
    {
        ...
        this->ks->hardRefServer->RegisterHardRef(this->ks, (nHardRef<nRoot> *)this);
    }
    else
    {
...
/**.
    ...
    @return         return pointer to name, or NULL if target object has died(???)
...
nHardRef<TYPE>::getname() const
{
    if (this->isvalid())
    {
        return this->o->GetFullName().c_str();
    }
    const char* name = this->strNode.GetName();
    if (name)
    {        
        return this->strNode.GetName();
    }
    return 0;
}
