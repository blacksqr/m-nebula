...
public:
    ...
    /// Class constructor
    nKernelServer(int argc, const char** argv);
    /// Class destructor
    ...
    virtual nRoot *New(const char *cl_name, const char *path, bool ahead=false); // Change by Zeev Clockwise
    virtual nRoot *NewNoFail(const char *cl_name, const char *path, bool ahead=false); // Change by Zeev Clockwise
    virtual nRoot *Load(const char *path);
    
    virtual nRoot *Lookup(const char *path);
    /// like a Lookup but check for class
    ///- 20-Jul-2006 ilya Kliot    created
    virtual nRoot *LookupClass(const char *path, const char* classname);
    
    virtual nRoot *SetParent(nRoot* thisObj, nRoot* toParent);
	...
	// argc, argv
	/// returns argc passed to main
	int GetArgc() const { return this->argc; }
	/// returns argv passed to main or 0 if index overrrun
	const char* GetArgv(int i) const { return (i < this->argc ? this->argv[i].c_str() : 0); }

    // current working object
    virtual nRoot *SetCwd(nRoot *);
    virtual nRoot *SetCwd(const char *);
    virtual nRoot *GetCwd(void);
    virtual void PushCwd(nRoot *);
    virtual nRoot *PopCwd(void);
	virtual void ReplaceCwdInStack(nRoot* old_node);
	...
	/// updates fileServer pointer to nFileServer2, allows to change file server to descendant class
	void   UpdateFileServer(nFileServer2* fs);
	void   SetQuitRequest() {this->quit_request++;}
	long   GetQuitRequest() const {return this->quit_request;}
...
private:
    ...
    nRoot *_checkCreatePath(const char *, const char *, bool, bool ahead=false); // Change by Zeev Clockwise
