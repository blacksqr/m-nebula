...
/**
    ...
    @brief Hold table of content entries for npk files.
...
private:
    ...
    stl_string	 rootPath;           // filesystem path to the root entry (i.e. d:/nomads)
...
nNpkToc::nNpkToc() :
    ...
    curStackIndex(0)
{
    n_memset(this->stack, 0, sizeof(this->stack));
}
...
nNpkToc::FindEntry(const char* absName)
{
    ...
    bool dotroot = !strcmp(this->rootDir->GetName(),".");    
    ...
    char* curComponent = ".";
    if (!dotroot)
        curComponent = strtok(tmpName, "/\\");

    nNpkTocEntry* curEntry = this->rootDir;
    if (!strcmp(curEntry->GetName(), curComponent))
    {
        while (curEntry && (curComponent = (dotroot ? strtok(tmpName, "/\\") : strtok(0, "/\\"))))
        {
            dotroot = false;
