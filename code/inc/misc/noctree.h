...
class nOctNode;
class N_INLINECLASS nOctElement : public nNode {
public:
...
public:
    ...
    virtual void RemElement(nOctElement *);
    virtual void UpdateElement(nOctElement *, const vector3&, float);
    virtual void BalanceTree(void);

    virtual int CollectByViewvol(matrix44&, matrix44&, nOctElement **, int); 
    ...
    virtual int CollectByFrustum(nGfxServer* gfx_server, nOctElement**, int);

    virtual void Visualize(nGfxServer *);

protected:
    nOctNode *allocnode(nOctNode *p, float x0, float x1, float y0, float y1, float z0, float z1);
