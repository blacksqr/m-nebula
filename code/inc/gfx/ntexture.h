...
public:
    ...
    int GetHeight();
    /// get depth of texture in pixels
    int GetDepth();
    /// get bytes per pixel
	...
	/// load texture from memory by pointer and size of BMP
	bool SetTexture( void* p, unsigned long size, long maxw = 256, long maxh = 256 );

    /// load texture from memory
    virtual bool loadFromMem(void* p, unsigned long size, long maxw = 256, long maxh = 256);
...
protected:
    ...
    /// + by Ilya Kliot, load texture from memory support        
    bool            memTex;
    void*           memTexPtr;
    unsigned long   memTexSize;    
    long   memMaxW;
    long   memMaxH;
