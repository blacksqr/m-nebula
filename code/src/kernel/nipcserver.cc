...
{
    sockaddr_in my_addr;
    n_memset(&my_addr, 0, sizeof(my_addr));
...
{
    ushort pnum = ((ushort)hash(portName, N_SOCKET_PORTRANGE)) + N_SOCKET_MIN_PORTNUM;
    return pnum;
} 
...
nIpcServer::nIpcServer(const char *portName)
{
    ...
    this->pname = portName;
    n_memset(&(this->hostAddr), 0, sizeof(this->hostAddr));
    ...
    n_memset(&(this->hostAddr.sin_zero), 0, sizeof(this->hostAddr.sin_zero));
    
    bool bound = false;
    ushort portNum = (ushort)this->GetPortNumFromName(portName);
    while (!bound) 
    {
...
*/
bool 
nIpcServer::AnswerMsg(const char* msg, int clientId)
{
    n_assert(msg);    
    n_assert(clientId >= 0);
    bool retval = false;
...
*/
bool
nIpcServer::BroadcastMsg(const char* msg, bool backline_only)
{
	bool res = true;
    
    this->miniServerList.Lock();
    for (nIpcMiniServer* ims = (nIpcMiniServer*)this->miniServerList.GetHead();
         ims;
         ims = (nIpcMiniServer*) ims->GetSucc())
    {		
		if ((backline_only && ims->IsBackLine()) || !backline_only)
			res = ims->Send(msg, backline_only) && res;
    }
    this->miniServerList.Unlock();
	return res;
}
...
*/
bool
nIpcServer::BroadcastData(const char* buf, int sz, bool backline_only)
{
	bool res = true;	 
	this->miniServerList.Lock();
	for (nIpcMiniServer*ims = (nIpcMiniServer*)this->miniServerList.GetHead();
	...
	{
		if ((backline_only && ims->IsBackLine()) || !backline_only)
			res = ims->SendData(buf, sz, backline_only) && res;
	}
	this->miniServerList.Unlock();
	return res;
}
