...
*/
bool
nRemoteServer::Broadcast(const char* str, bool backline_only)
{
    n_assert(str);
    if (this->isOpen)
    {
        n_assert(this->ipcServer);
        return this->ipcServer->BroadcastMsg(str, backline_only);
    }
	return false;
}
...
*/
bool 
nRemoteServer::BroadcastData(const char* buf, int sz, bool backline_only)
{
	n_assert(buf);
	if (this->isOpen)
	{
		n_assert(this->ipcServer);
		return this->ipcServer->BroadcastData(buf, sz, backline_only);
	}
	return false;
}
		...
		if (this->ipcServer->Poll())        
        {
            ...
            while ((msg = this->ipcServer->GetMsg(msgClientId)))
            {
                ...
                if ((msgSize > 0) && (msgPtr[msgSize - 1] == 0))
                {
                    ...
                    this->ipcServer->AnswerMsg(answerMsg, msgClientId);
                ...
                else
                {
                    ...
                    this->ipcServer->AnswerMsg(msg, msgClientId);
                }
            }
        }
