...
/*
    ...
    FIXME: memlog temporarily disabled, since it clutters the working directory
...
//-------------------------------------------------------------------
/**
    @brief Allocate a new chunk and return a pointer to the first
    block in the chunk.
...
//-------------------------------------------------------------------
/**
    @brief Release an allocated chunk.
...
void *nn_calloc(size_t num, size_t size, const char *file, int line)
{
    ...
    if (p) n_memset(p,0,sizeof(all_size));
    return p;
}
#else
