...
#include <windows.h>
#include <direct.h>
# pragma comment (lib, "user32.lib")  // Add by Clockwise
#endif
...
*/
N_EXPORT wchar* n_wstrdup(const wchar* from)
{
    wchar* to = (wchar*) n_malloc((wcslen(from)+1) * sizeof(wchar));
    if (to) wcscpy(to, from);
    return to;
}

//-----------------------------------------------------------------------------
/**
  - 26-Nov-2008   ilya    created
*/
N_EXPORT wchar* n_str2wstr(const char* str)
{
	...
	if (str)
	{
		...
		if (str_len)
		{
			int wstr_len = mbstowcs(0, str, 0) + 1;

			wstr = new wchar[wstr_len];
			if (wstr)
			{				
				n_memset (wstr, 0x00, sizeof (wchar)*wstr_len);
				mbstowcs(wstr, str, wstr_len);				
			}
		}		
	}
...
/**
  ...
  is terminated correctly (that is, in ALL cases, a terminating
...
/**
  ...
  @param dest A char pointer where the string will be concatenated  
