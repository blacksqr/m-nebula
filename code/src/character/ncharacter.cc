...
nCharacter::nCharacter() :
    ...
    activeState(0)
{
    n_memset(this->activeStateChannels, 0, sizeof(this->activeStateChannels));
}
...
*/
void
nCharacter::RenderSkeleton(nSceneGraph2* sceneGraph, nPrimitiveServer* primServer, vector4& color)
{
    if (primServer) {

		nPixelShader* ps = sceneGraph->GetPixelShader();
        // save old state
        nRStateParam zf = ps->GetShaderDesc()->GetZFunc();
        // set zfunc to always, so that skeleton overlays skin.
        ps->GetShaderDesc()->SetZFunc(N_CMP_ALWAYS);
