...
nLenseFlare::BeginFlares(int iNumFlares)
{
    ...
    this->iNumberOfFlares = (ushort)iNumFlares;

    nFlareDesc tmp;
    aFlares = new nFlareDesc[this->iNumberOfFlares];
    for (int i = 0; i < this->iNumberOfFlares; i++)
        aFlares[i] = tmp;
...
{
    n_assert(bBegun == true);
    n_assert(aFlares != 0 && ndx < this->iNumberOfFlares);
...
nLenseFlare::Compute(nSceneGraph2* sceneGraph)
{
    ...
    if (bEnded)
    {
        ...
        if (bWithinBounds)
        {
            ...
            this->ref_dynvbuf.End(this->iNumberOfFlares * 4, ref_ibuf.get()->GetNumIndices());
...
nLenseFlare::InitVBuffer()
{
    ...
    ushort iNumberOfVertices = this->iNumberOfFlares * 4;    // 4 vert's per quad (flare)
    ...
    if (!ibuf)
    {
        ...
        ushort iNumberOfIndices = this->iNumberOfFlares * 6;     // 6 indices per quad
        ibuf->Begin(N_IBTYPE_STATIC, N_PTYPE_TRIANGLE_LIST, iNumberOfIndices);

        ushort indexIndex  = 0;         
        for (ushort vertexIndex = 0; vertexIndex < this->iNumberOfFlares * 4; vertexIndex += 4)
        {
...
nLenseFlare::UVFlares()
{
    ...
    int iCnt = 0;
    for (int i = 0; i < this->iNumberOfFlares * 4; i += 4, iCnt++)
    {
        n_assert(iCnt < this->iNumberOfFlares);
