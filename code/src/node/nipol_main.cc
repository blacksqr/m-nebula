...
nIpol::Compute(nSceneGraph2 *sceneGraph)
{
    ...
    if (this->keyArray.size() > 1)
    {
        ...
        if (max_t > 0.0)
        {
            ...
            // find the right keys
            int i = 0;
            if (n_neqz(this->keyArray.front().t))
            {                
                n_error("Object '%s' 1st keyframe > 0.0f (%f)!\n", this->GetFullName().c_str(), this->keyArray.front().t);
            }

            while ((this->keyArray[i].t * tscale) <= t) i++;
            n_assert((i > 0) && (i < (int)this->keyArray.size()));
            nIpolKey& k0 = this->keyArray[i - 1];
            nIpolKey& k1 = this->keyArray[i];
                ...
                case N_SPLINE:
                    {
                        ...
                        if (i == 1)
                        {
                            kA = &(this->keyArray[i-1]);
                            tA = k0.t - (k1.t - k0.t);
                        }
                        else
                        {
                            kA = &(this->keyArray[i-2]);
                            tA = kA->t;
                        }

                        nIpolKey *kB; // key right to i
                        if(i == (int)this->keyArray.size() - 1)
                        {
                            kB = &(this->keyArray[i]);
                            tB = k1.t + (k1.t - k0.t);
                        }
                        ...
                        {
                            kB = &(this->keyArray[i + 1]);
                            tB = kB->t;
                        }

                        float l2 = l*l;
                        float l3 = l2*l;
                        float k0t_tA = 1.0f/(k0.t - tA);
                        float k1t_k0t = 1.0f/(k1.t - k0.t);
						...
						for (int i = 0; i < 4; i++)
						{
							k_0 = n_half(k0.f[i] - kA->f[i]) * (k0t_tA) + n_half(k1.f[i] - k0.f[i]) * (k0t_tA);
							k_1 = n_half(k1.f[i] - k0.f[i]) * (k1t_k0t) + n_half(kB->f[i] - k1.f[i]) * (k1t_k0t);
							this->curval[i] = k0.f[i] * (2.0f*l3 - 3.0f*l2 + 1.0f) + k1.f[i] * (3.0f*l2 - 2.0f*l3) +
											 k_0 * (k1t_k0t) * (l3 - 2.0f*l2 + l) +
											 k_1 * (tB_k1t) * (l3 - l2);
						}                        
                    }
                    break;
