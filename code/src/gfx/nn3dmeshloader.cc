...
*/
void
nN3dMeshLoader::createTriList(ushort* indices, int num_indices, bool tri_strip)
{
    ...
    if (num_indices > 0) 
	{
        if (tri_strip) 
		{
            this->indexBuffer->Begin(N_IBTYPE_STATIC, N_PTYPE_TRIANGLE_STRIP, num_indices);            
        } 
		else  
		{
...
nN3dMeshLoader::Load(const char* fileName)
{
    ...
    while (file->GetS(line, sizeof(line)))
    {
        ...
        if (kw) 
        {
            ...
            else if (strcmp(kw, "f")    == 0) 
            {
                ...
                n_assert(!tri_strip && "ERROR: mixed primitives type, tri-list after tri-strip");
            ...
            {
                // a triangle strip begin definition
                n_assert(!tri_strip && "ERROR: only 1 strip can be defined");
                n_assert(num_indices == 0 && "ERROR: mixed primitives type, tri-strip after tri-list");
                tri_strip = true;
                num_primitives = 1;
                num_indices += 3;
            } 
            ...
            {
                // a triangle strip definition
                n_assert(tri_strip && "ERROR: strip head missed");
                num_primitives = 1;
                num_indices ++;
            } 
        }
	...
	while (file->GetS(line, sizeof(line))) 
    {
		...
		if (kw) 
        {
            ...
            else if ((strcmp(kw, "rgba")==0) && (vertexType & N_VT_RGBA)) 
            {
				...
				((strcmp(kw, "vt") == 0) && (vertexType & N_VT_UV0)) ||
				((strcmp(kw,"vt1") == 0) && (vertexType & N_VT_UV1)) ||
				((strcmp(kw,"vt2") == 0) && (vertexType & N_VT_UV2)) ||
				((strcmp(kw,"vt3") == 0) && (vertexType & N_VT_UV3))
			)
            {
				int i = kw[2] ? kw[2] - '0' : 0;
	            char* us = strtok(NULL, " \t\n");
                ...
                if (us && vs) 
                {
					...
					this->vertexBuffer->Uv(act_vt++, i, v);
				} 
                else 
                {
            ...
            else if (strcmp(kw, "we")==0)
            {
                ...
                {
                    ushort v0  = (ushort)atoi(v0s);
                    ushort v1  = (ushort)atoi(v1s);
                    ushort vp0 = (ushort)atoi(vp0s);
                    ushort vp1 = (ushort)atoi(vp1s);
            ...
            else if (strcmp(kw,"f")==0) 
            {
				for (i=0; i<3; i++) 
                {
					...
					if (tmp) 
                    {
						indices[num_indices++] = (ushort)atoi(tmp)-1;
					}
				}
            }
            else if (strcmp(kw,"t")==0) 
    ...
    this->createTriList(indices, num_indices, tri_strip);
