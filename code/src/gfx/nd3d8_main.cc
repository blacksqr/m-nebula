...
/**
	...
	+ 01-Dec-2002	Ilya,   compressed textures support, Clockwise  Ltd
*/
    ...
    parent_hWnd(0)
{
    ...
    n_memset(&this->presentParams,    0, sizeof(this->presentParams));
    n_memset(&this->devCaps,          0, sizeof(this->devCaps));
    n_memset(&this->vpoolManager,     0, sizeof(this->vpoolManager));
    n_memset(&this->d3dWorldMatrix,   0, sizeof(this->d3dWorldMatrix));
    n_memset(&this->d3dProjMatrix,    0, sizeof(this->d3dProjMatrix));

	//  01-Dec-2002 Ilya,   + compressed textures support, Clockwise  Ltd
	this->texture_compression = false;
	n_memset((void*)this->s3tc_formats, 0, sizeof(bool)*5);

	// + 25-Mar-2003 anisotropic textures filtering support, by Ilya Kliot, Clockwise  Ltd
	this->anisotropic_filter = false;
	this->anisotropic_min_filter = false;
	this->anisotropic_mag_filter = false;
...
void 
nD3D8Server::Resize(int x, int y, int w, int h)
{
	if (this->hWnd)
	{		
