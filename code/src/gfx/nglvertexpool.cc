...
void nGlVertexPool::UnlockBubble(nVertexBubble *vb)
{
	...
	this->norm_ptr  = NULL;
	this->color_ptr = NULL;
	n_memset(uv_ptrs,0,sizeof(uv_ptrs));
}
...
*/
    ...
    int num_triangles = 0;    
    for (int current_pass=0; current_pass<num_passes; current_pass++) {
        ...
        if (ps) 
		{
			...
			if (this->vtype & (N_VT_UV0|N_VT_UV1|N_VT_UV2|N_VT_UV3)) {
				...
				for (j=0; j<glps->pass_map_num[current_pass]; j++) {
					...
					if (glClientActiveTextureARB) glClientActiveTextureARB(GLenum(GL_TEXTURE0_ARB + j));
#else
...
#error "ARB_multitexture not supported by compile environment!"
		...
		if (glLockArraysEXT) glLockArraysEXT(0, vb_size);
#endif	
