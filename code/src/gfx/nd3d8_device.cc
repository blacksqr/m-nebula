...
//-----------------------------------------------------------------------------
/**
    check is specified format supported by driver
*/
bool 
nD3D8Server::IsTextureFormatSupported( D3DFORMAT TextureFormat, 
									   D3DFORMAT AdapterFormat, D3DRESOURCETYPE type) {
    HRESULT hr = this->d3d8->CheckDeviceFormat( D3DADAPTER_DEFAULT,
		D3DDEVTYPE_HAL,
		AdapterFormat,
		0,
		type,	
	...
	if (SUCCEEDED( hr )) {
		...
		long index = GetS3TCFormatIndex(TextureFormat);
		if (index >= 0) {
			char  fourcc[5];
			char* fccp = (char*)&(TextureFormat);
			fourcc[0] = fccp[0];
			fourcc[1] = fccp[1];
			fourcc[2] = fccp[2];
			fourcc[3] = fccp[3];
			fourcc[4] = '\0';
			n_printf("nD3D8Server: texture format %s supported\n", fourcc); 
			this->texture_compression = true;
			this->s3tc_formats[index] = true;			
        }		
		return true;
	} else 
		return false;	
...
*/
void 
nD3D8Server::QueryFormats() {
	D3DDISPLAYMODE dispMode;
	this->d3d8Dev->GetDisplayMode(&dispMode);	
	this->IsTextureFormatSupported(D3DFMT_DXT1, dispMode.Format);
	this->IsTextureFormatSupported(D3DFMT_DXT2, dispMode.Format);
	this->IsTextureFormatSupported(D3DFMT_DXT3, dispMode.Format);
	this->IsTextureFormatSupported(D3DFMT_DXT4, dispMode.Format);
	this->IsTextureFormatSupported(D3DFMT_DXT5, dispMode.Format);
	this->IsTextureFormatSupported(D3DFMT_DXT5, dispMode.Format);
	this->IsTextureFormatSupported(D3DFMT_A8R8G8B8, dispMode.Format, D3DRTYPE_VOLUMETEXTURE );
}
...
nD3D8Server::initDeviceStates()
{
    ...
    n_memset(&identity,0,sizeof(identity));
...
nD3D8Server::devOpen()
{
    ...
    if (this->windowFullscreen)
    {
        ...
        nEnv* zbufEnv = (nEnv*) this->refSelMode->Find("zbuf");
        nEnv* sbufEnv = (nEnv*) this->refSelMode->Find("sbuf");
		...
		this->disp_zbufbits = zbufEnv->GetI();
		this->disp_sbufbits = sbufEnv->GetI();            
    ...
    else
    {
        ...
        if (this->bppFromFormat(dispFormat) >= 24) {
			this->disp_zbufbits = 24;
			this->disp_sbufbits = 8;
	...
	if (this->devCaps.RasterCaps & D3DPRASTERCAPS_ANISOTROPY) {
		if (this->devCaps.MaxAnisotropy > 1) {
			this->anisotropic_filter = true;
			this->max_anisotropy = this->devCaps.MaxAnisotropy;
			this->anisotropic_mag_filter = (this->devCaps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFANISOTROPIC) != 0;
			this->anisotropic_min_filter = (this->devCaps.TextureFilterCaps & D3DPTFILTERCAPS_MINFANISOTROPIC) != 0;
		}
	}
	...
	this->QueryFormats();	
	n_printf("nD3D8Server: texture compression%ssupported\n", this->texture_compression ? " " : " not ");     
...
nD3D8Server::ShowCursor()
{
    ...
    nGfxServer::SetCursorShown(true);
}
...
nD3D8Server::HideCursor()
{
    ...
    nGfxServer::SetCursorShown(false);
}
