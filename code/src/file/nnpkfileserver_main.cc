...
nNpkFileServer::ParseFile(const char* pathName)
{
    ...
    this->MakeAbsoluteMangled(pathName, absPath);

	stl_string rootPath(absPath);
    ...
    // read file...
    if (this->ParseNpkFile(rootPath.c_str(), absPath.c_str()))
    {
        return true;
    }
    else
...
nNpkFileServer::ChangeDir(const char* newDir)
{
    ...
    stl_string absPath;    
    nNpkTocEntry* entry = this->FindFirstTocEntry(this->MakeAbsoluteMangled(newDir, absPath));
