#define N_IMPLEMENTS nNpkDirectory
//------------------------------------------------------------------------------
//  nnpkdirectory.cc
...
nNpkDirectory::Open(const char* dirName)
{
    ...
    this->tocEntry = this->npkFileServer->FindFirstTocEntry(this->npkFileServer->MakeAbsoluteMangled(dirName, this->apath));
...
nNpkDirectory::SetToFirstNpkEntry()
{
    ...
    this->tocEntry = this->npkFileServer->FindFirstTocEntry(this->apath.c_str());
...
nNpkDirectory::SetToNextNpkEntry()
{
    ...
    if (0 == this->curSearchEntry)
    {
        // current npk file exhausted, try next npk file
        this->tocEntry = this->npkFileServer->FindNextTocEntry(this->tocEntry, this->apath.c_str());
        if (this->tocEntry)
        {
            this->curSearchEntry = this->tocEntry->GetFirstEntry();
        }
...
nNpkDirectory::CheckNpkEntryOverride()
{
    ...
    {
        // intercept!
        this->curSearchEntry = this->npkFileServer->FindFirstTocEntry(entryName);
