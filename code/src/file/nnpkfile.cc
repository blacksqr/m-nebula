...
nNpkFile::Open(const char* filename, const char* accessMode)
{
    ...
    // + by Ilya Clockwise, check file first only in developer mode
    if (this->fs->GetDevMode()) 
    {
	...
	stl_string absPath;	
    this->tocEntry = this->npkFileServer->FindFirstTocEntry(this->npkFileServer->MakeAbsoluteMangled(filename, absPath));
    if (this->tocEntry && (nNpkTocEntry::FILE == this->tocEntry->GetType()))
    {
        ...
        // + by Ilya Clockwise, check file after fail when not in developer mode
        if (!this->fs->GetDevMode()) 
        {
            // try to open as conventional file
            if (nFile::Open(filename, accessMode)) 
