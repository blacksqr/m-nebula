...
if {[file exists packages] == 0} {
	file mkdir packages
}

if {[file exists ../vstudio] == 0} {
	file mkdir ../vstudio
}
...
foreach {ws wsl} [array get workspaces] {
    ...
    #we don't need project file for visual studio 6
    ##gen_workspace $ws $wsl
    
    # Build VS7 solution files.
    #foreach file [glob -nocomplain -directory ../vstudio *{.vcproj,.sln} ]  {
    #    puts "check out $file"
    #    set status 0
    #    if {[catch {exec cleartool co -comment "automatic checkout" -nwa -nq $file} results]} {
