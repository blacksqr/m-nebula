...
#define N_TCLSERVER_SCRIPTS  "/sys/share/tcl_scriptlets"
...
void nTclServer::link_to_interp(Tcl_Interp *interp, bool is_standalone)
{
    ...
    if (is_standalone) {
        ...
        Tcl_SetVar(this->interp, "tcl_library", path.c_str(), 0);
        #endif
    ...
    refFileServer(kernelServer, this)
{
	...
	kernelServer->New("nroot",N_TCLSERVER_SCRIPTS); 
	this->extensions.push_back(stl_string("tcl"));
	this->extensions.push_back(stl_string("ntcl"));
}
