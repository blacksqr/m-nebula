...
/**
    ...
    @brief nVisNode wrapper around nPixelShader
  	...
  	25-Mar-2003 Ilya 	+ Anisotropic textures filtering support
*/
//--------------------------------------------------------------------
#ifndef N_VISNODE_H
...
public:
    ...
    //--- render pri ---
    void SetRenderPri(short p) 
    {
        this->render_pri = p;
    };
    short GetRenderPri(void) 
    {
	...
	void SetAddress3D(nRStateParam tau, nRStateParam tav, nRStateParam taw) 
    {
        n_assert(in_begin_tunit);
        ps_desc.SetAddressU(cur_tunit,tau);
        ps_desc.SetAddressV(cur_tunit,tav);
		ps_desc.SetAddressW(cur_tunit,taw);
    };
    ...
    void SetConst(int tunit, const color4& v) 
    {
	...
	void SetAnisotropicFilter(bool b) {
		ps_desc.SetAnisotropicFilter(cur_tunit, b);		
	};
    ...
    {
        n_assert(in_begin_tunit);
        ps_desc.SetEnable3D(cur_tunit,b);
    };
	...
	// + 14-Jun-2004 3d textures support, by Ilya Kliot, Insightec Ltd
	void GetAddress3D(int stage, nRStateParam& tau, nRStateParam& tav, nRStateParam& taw) 
    {
        tau = ps_desc.GetAddressU(stage);
        tav = ps_desc.GetAddressV(stage);
		taw = ps_desc.GetAddressW(stage);
    };
	...
	bool GetAnisotropicFilter(int stage) {
		return ps_desc.GetAnisotropicFilter(stage);		
	};
    ...
    {
        return ps_desc.GetEnable3D(stage);
    };
    ...
    {
        ps_desc.SetSpecular(v);
    };
    void SetShininess(float se) {
        ps_desc.SetShininess(se);        
    };
    void SetEmissive(color4& v) 
    {
        ps_desc.SetEmissive(v);
    };
    void SetAmbient(color4& v) 
    {
	...
	{
		ps_desc.SetStencilTestEnable(b);
	}
	void SetStencilFunc(nRStateParam p)
	{
		ps_desc.SetStencilFunc(p);
	}
	void SetStencilRef(uint r)
	{
		ps_desc.SetStencilRef(r);
	}
	void SetStencilMask(uint m)
	{
		ps_desc.SetStencilMask(m);
	}
	void SetStencilOp(nRStateParam sfail, nRStateParam zfail, nRStateParam pass)
	{
		ps_desc.SetStencilOp(sfail, zfail, pass);
	}
	void SetStencilWriteMask(uint wm)
	{
		ps_desc.SetStencilWriteMask(wm);
	}
    ...
    {
        return ps_desc.GetSpecular();
    };
    float GetShininess(void) {
        return ps_desc.GetShininess();
    };
    const color4& GetEmissive(void) 
    {
        return ps_desc.GetEmissive();
    };
    const color4& GetAmbient(void) 
    {
	...
	{
		return ps_desc.GetStencilTestEnable();
	}
	nRStateParam GetStencilFunc() const
	{
		return ps_desc.GetStencilFunc();
	}
	uint GetStencilRef() const
	{
		return ps_desc.GetStencilRef();
	}
	uint GetStencilMask() const
	{
		return ps_desc.GetStencilMask();
	}
	void GetStencilOp(nRStateParam& sfail, nRStateParam& zfail, nRStateParam& pass) const
	{
		ps_desc.GetStencilOp(sfail, zfail, pass);
	}
	uint GetStencilWriteMask() const
	{
		return ps_desc.GetStencilWriteMask();
	}
    /// set mipmap lod bias
