...
public:
    ...
    const matrix44& GetParentTransform() const;
    
	/// set the current node's object 2 world transform
	void SetO2WTransform(const matrix44&);
	/// get the current node's object 2 world transform
	const matrix44& GetO2WTransform() const;
	/// get the current node's object 2 world parent transform
	const matrix44& GetParentO2WTransform() const;
	
	/// Set original node pointer for current node
	void SetOrigin(nVisNode* o);
	/// Get original node pointer of current node
	nVisNode* GetOrigin() const;	
	...
	/// get pointer to gfx server  + by Ilya Dec-2007
	nGfxServer* GetGfxServer() {return this->refGfx.get();}
...
nSceneGraph2::NewNodeFromPool(short parentNode)
{
    ...
    else
    {
        ...
        n_memset(&node, 0, sizeof(nSceneNode));
...
{
    n_assert((this->curNode >= 0) && (this->curNode < MAX_NODES));
	n_bsetbit(opaq, nSceneNode::ISOPAQUE, this->nodePool[this->curNode].flags);
}
...
inline
const matrix44&
nSceneGraph2::GetTransform() const
{n_assert((this->curNode >= 0) && (this->curNode < MAX_NODES));
...
inline
const matrix44&
nSceneGraph2::GetParentTransform() const
{
    n_assert((this->curNode >= 0) && (this->curNode < MAX_NODES));
    const nSceneNode& node = this->nodePool[this->curNode];
...
//------------------------------------------------------------------------------
/**
	Set the transformation matrix of object to world 
	for the current scene graph node. This is 
	normally done inside n3DNode::Compute().

	@param  matrix  a 4x4 object to world matrix
*/
inline
void
nSceneGraph2::SetO2WTransform(const matrix44& matrix)
{
	n_assert((this->curNode >= 0) && (this->curNode < MAX_NODES));
	this->nodePool[this->curNode].node2world = matrix;
}


//------------------------------------------------------------------------------
/**
	Get the object to world transformation for the current scene 
	graph node.

	@return         a const ref to the current node's 4x4 object to world matrix
*/
inline
const matrix44&
nSceneGraph2::GetO2WTransform() const
{
	n_assert((this->curNode >= 0) && (this->curNode < MAX_NODES));
	return this->nodePool[this->curNode].node2world;
}

//------------------------------------------------------------------------------
/**
	Get the parent node's object to world transform.

	@return     the parent node's object to world transform
*/
inline
const matrix44&
nSceneGraph2::GetParentO2WTransform() const
{
	n_assert((this->curNode >= 0) && (this->curNode < MAX_NODES));
	const nSceneNode& node = this->nodePool[this->curNode];
	n_assert((node.parentNode >= 0) && (this->curNode < MAX_NODES));
	return this->nodePool[node.parentNode].node2world;
}

//------------------------------------------------------------------------------
/**
	Set the pointer to origin node in the current scene graph node. This is 
	normally done inside n3DNode::Compute().

	@param  pointer to nVisNode
*/
inline
void
nSceneGraph2::SetOrigin(nVisNode* o)
{n_assert((this->curNode >= 0) && (this->curNode < MAX_NODES));
	this->nodePool[this->curNode].origin = o;
}

//------------------------------------------------------------------------------
/**
	Get the origin of the current scene graph node. 

	@return nVisNode
*/
inline
nVisNode*
nSceneGraph2::GetOrigin() const 
{n_assert((this->curNode >= 0) && (this->curNode < MAX_NODES));
	return this->nodePool[this->curNode].origin;
}
