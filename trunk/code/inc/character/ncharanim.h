...
{
    return this->weightChannelName.empty() ? 0 : this->weightChannelName.c_str();
}
...
{
    return this->animName.empty() ? 0 : this->animName.c_str();
}
...
nCharAnim::SetParams(const char* stateName, const char* aName, const nAnimCurveArray* curveArray, const nCharSkeleton* skel)
{
    ...
    for (i = 0; i < this->numCurves; i++)
    {
        ...
        sprintf(transBuf, "%s_%s_trans_%s", stateName, this->animName.c_str(), jointName);
        sprintf(rotBuf, "%s_%s_rot_%s", stateName, this->animName.c_str(), jointName);
