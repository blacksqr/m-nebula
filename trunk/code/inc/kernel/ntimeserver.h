...
public:
	/// + by Ilya, internal timers support
	class nTimer : public nStrNode
	{
	public:
		nTimer(const char *name, double start) : nStrNode(name)
		{
			this->reg_time = start;
		}
		double		delay;
		...
		stl_string	command;
		stl_string	param;
		bool		check(double time)		{ return (time - this->reg_time >= this->delay); }
		double		remainder(double time)  { return (this->delay - (time - this->reg_time)); }
	};
protected:
    bool stopped;
    bool frame_enabled;
    double frame_time;
    float  frame_time_scale; // Add by Zeev Clockwise - scale the time frame.
    double lock_delta_t;
    double wait_delta_t;
    double lock_time;
	bool   asleep;			// support for dynamic sleep management, + by Ilya, May 2008

	/// list of living nProfiler objects
    nStrList prof_list;   
	/// list of timers + by Ilya, apr-2008
	nStrList timers_list;
...
public:
    ...
    */
	...
	/**
		@name Timers support
		+ by Ilya
		Manage nTimer objects.  See the documentation for nTimer
		for further details.
	*/
	//@{
	virtual nTimer* NewTimer(const char*, float, const char*, const char*);
	virtual void ReleaseTimer(nTimer*);
	virtual bool ReleaseTimer(const char*);
	virtual double GetTimerRemainder(const char*);
	virtual nStrList* GetTimers(void);
	//@}
    /**
    ...
    */
	...
	/// support for dynamic sleep management, + by Ilya, May 2008
	virtual void SetSleep(bool b) { this->asleep = b; }
	virtual bool GetSleep() const { return this->asleep; }
    //@}
    ...
    */
    ...
    virtual float  GetScale(void); // Add by Zeev Clockwise - scale the time frame.
    virtual void   SetScale(float); // Add by Zeev Clockwise - scale the time frame.
