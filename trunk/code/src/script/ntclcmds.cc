...
#define MAX_ARGS (64)
	if (objc >= MAX_ARGS)
	{
		n_printf("tcl_pipe_command: %s has too much args %d\n", cmd, objc);
	}
...
int tclcmd_New(ClientData cdata, Tcl_Interp *interp, int objc, Tcl_Obj *CONST *objv)
{
    ...
    if (objc != 3) tcl_staticError(interp, tcl->print_error, "Syntax is 'name = new class name'");
        ...
        if (o) 
		{            
            Tcl_SetResult(interp, (char*)o->GetFullName().c_str(), TCL_VOLATILE);
            retval = TCL_OK;
        }
    }
    return retval;
...
int tclcmd_Delete(ClientData cdata, Tcl_Interp *interp, int objc, Tcl_Obj *CONST *objv)
{
    ...
    if (objc != 2) tcl_staticError(interp, tcl->print_error, "Syntax is 'delete name'");
		...
		{
			tcl_unknownObject(interp,tcl->print_error,Tcl_GetString(objv[1]));
    ...
    if (num_args == objc) {
        ...
        for (i=0; i<num_args; i++) {
            ...
            switch(arg->GetType()) {
				...
				{
					const wchar* str = (wchar*)Tcl_GetUnicode(objv[i]);
					if (wcscmp(L":null:", str) == 0) 
					{
						str = NULL;
					}
					arg->SetU(str);
        ...
        switch(arg->GetType()) {
			...
			case nArg::ARGTYPE_USTRING:
				{
					...
					Tcl_Obj* so = Tcl_NewUnicodeObj((const Tcl_UniChar*)s, wcslen(s));
					Tcl_ListObjAppendElement(interp, res, so);
				}
				break;
            ...
            case nArg::ARGTYPE_OBJECT:
                {   
					...
					const char* s = (o ? o->GetFullName().c_str() : "null");
			...
			case nArg::ARGTYPE_USTRING:
				{
					...
					Tcl_SetUnicodeObj(res, (const Tcl_UniChar*)s, wcslen(s));
				}
				break;
...
{
    char error_buf[1024];    
    sprintf(error_buf, msg, o->GetFullName().c_str(), cmd_name);
