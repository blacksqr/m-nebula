...
void doMouse( nInputEvent* ie, Point where, int disp_w, int disp_h )
{
	ie->SetDeviceId( N_INPUT_MOUSE( 0 ) );
	ie->SetAbsPos( where.h, where.v );
	float relX = float( where.h ) / disp_w;
	float relY = float( where.v ) / disp_h;
	...
	if ( result != noErr )
	{
		...
		if ( gls->ref_is.isvalid() )
		{
			...
			if ( ie )
			{
				...
				doMouse( ie, where, gls->disp_w, gls->disp_h );
