...
nDepthStencilFormat g_DepthStencilFormats[] = {
    { D3DFMT_D24S8,     32, 8, false },    // try format with stencil buffer first
    { D3DFMT_D24X4S4,   32, 4, false },
    { D3DFMT_D32,       32, 0, false },    // then, others
    { D3DFMT_D24X8,     32, 0, false },
    { D3DFMT_D16,       16, 0, true  },    // in 16/15bit zbuffer try to use Wbuffer (to avoid z buffer artifacts)
    { D3DFMT_D15S1,     16, 1, true },
    { D3DFMT_UNKNOWN,   0,  0, false }
};
...
nD3D8Server::enumDisplayModes(int adapter)
{
    ...
    n_memset(&prev_dmode, 0, sizeof(prev_dmode));
    n_memset(&dmode, 0, sizeof(dmode));
                 ...
                 (dmode.Format == prev_dmode.Format)))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    while (g_DepthStencilFormats[i].dsFormat != D3DFMT_UNKNOWN) 
                    {
                        ...
                        int sbuf_bpp = g_DepthStencilFormats[i].sbpp;
                        ...
                        if (this->checkZBufferFormat(adapter, dmode.Format, zbuf_format))
                        {
                            ...
                            sprintf(buf, "type(full)-w(%d)-h(%d)-bpp(%d)-zbuf(%d)-sbuf(%d)",
	                                dmode.Width, dmode.Height, bpp, zbuf_bpp, sbuf_bpp);                            
							...
							env = (nEnv *) kernelServer->New("nenv", "sbuf");
	                        env->SetI(sbuf_bpp);
