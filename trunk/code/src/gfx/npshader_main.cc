...
nNebulaClass(nPixelShader, "nshaderprogram");

//-------------------------------------------------------------------
/**
    - 23-Aug-00   floh    created
    - 26-May-02   zeev    fix preload bug.
*/
//-------------------------------------------------------------------
...
nPixelShader::~nPixelShader()
{	
    if (this->refGfx.isvalid() && this == this->refGfx->GetCurrentPixelShader()) {
        n_printf("nPixelShader::~nPixelShader(): clearing current pixel shader\n");
        this->refGfx->SetCurrentPixelShader(NULL);
    }
}
