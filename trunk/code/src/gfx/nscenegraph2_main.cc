    ...
    mipLodEnabled(false)
{
    ...
    n_memset(this->geoNodes,      0, sizeof(this->geoNodes));
    n_memset(this->lightNodes,    0, sizeof(this->lightNodes));
    n_memset(this->soundNodes,    0, sizeof(this->soundNodes));
    n_memset(this->nodeStack,     0, sizeof(this->nodeStack));
    n_memset(this->nodePool,      0, sizeof(this->nodePool));
}
...
nSceneGraph2::BeginScene(const matrix44& invViewer)
{
	...
	/*static matrix44 rootm;

	this->viewer_matrix_dirty = !rootm.isequal(invViewer, TINY);

	if (this->viewer_matrix_dirty)
	{
		rootm = invViewer;
	}
*/
