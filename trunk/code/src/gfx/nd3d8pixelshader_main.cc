...
{
    this->num_passes = 0;
    n_memset(this->pass_map_base,     0,  sizeof(this->pass_map_base));
    n_memset(this->pass_map_num,      0,  sizeof(this->pass_map_num));
    n_memset(this->state_block,       0,  sizeof(this->state_block));
    n_memset(this->spheremap_hint,    0,  sizeof(this->spheremap_hint));
    n_memset(this->objectspace_hint,  0,  sizeof(this->objectspace_hint));
}
...
nD3D8PixelShader::~nD3D8PixelShader()
{
    nD3D8Server *gs = (nD3D8Server *) this->refGfx.get();
    n_assert(gs->d3d8Dev);
...
nD3D8PixelShader::arg2d3d(nPSI::nArg arg)
{
    ...
    switch (arg & nPSI::ARGMASK) {
        case nPSI::TEX:       d3dta = D3DTA_TEXTURE; break;
        case nPSI::PREV:      d3dta = D3DTA_CURRENT; break;
        case nPSI::CONSTANT:  d3dta = D3DTA_TFACTOR; break;
        case nPSI::PRIMARY:   d3dta = D3DTA_DIFFUSE; break;
    }
    if (arg & nPSI::ONE_MINUS) d3dta |= D3DTA_COMPLEMENT;
    return d3dta;
}
...
/**
    ...
    - 08-Jan-03   Ilya    + Specular support
...
nD3D8PixelShader::emit_simple_states(nD3D8Server *gs, nPixelShaderDesc *psd)
{
    ...
    if (psd->specular_enable)   r=TRUE;
    else                        r=FALSE;
    dev->SetRenderState(D3DRS_SPECULARENABLE,r);
	...
	if (psd->normalizenormals_enable || gs->GetAutoNormalize()) {
    ...
    int stage)
{
	...
	D3DTEXTUREFILTERTYPE N_MIN_LINEAR = D3DTEXF_LINEAR;
	D3DTEXTUREFILTERTYPE N_MAG_LINEAR = D3DTEXF_LINEAR;

	if (gs->anisotropic_filter) {
		if (tu.anisotropic_filter) {
			dev->SetTextureStageState(stage, D3DTSS_MAXANISOTROPY, gs->max_anisotropy);
			if (gs->anisotropic_min_filter)
				N_MIN_LINEAR = D3DTEXF_ANISOTROPIC;
			if (gs->anisotropic_mag_filter)
				N_MAG_LINEAR = D3DTEXF_ANISOTROPIC;

		} else {
			dev->SetTextureStageState(stage, D3DTSS_MAXANISOTROPY, 1);
		}
	}
    ...
    switch(tu.coord_src) {
        ...
        case N_TCOORDSRC_SPHEREMAP:
            ...
            this->ps_desc->SetTransformChanged(true);
            break;
        case N_TCOORDSRC_REFLECTION: // Add by Clockwise zeev
            dev->SetTextureStageState(stage,D3DTSS_TEXCOORDINDEX,D3DTSS_TCI_CAMERASPACEREFLECTIONVECTOR|stage);
...
nD3D8PixelShader::translate_pixel_op(nPSI& psi, D3DCAPS8 *desc)
{
    ...
    switch (psi.op) {
        ...
        //--- ipol ---
        case nPSI::IPOL:
            switch (psi.args[2] & nPSI::ARGMASK) {
                case nPSI::TEX:
...
// n_printf("upd_lighting\n");
            ...
            memcpy(&(d3d8_mat.Specular),&(this->ps_desc->specular),4*sizeof(float));
            memcpy(&(d3d8_mat.Emissive),&(this->ps_desc->emissive),4*sizeof(float));
            d3d8_mat.Power = (this->ps_desc->specular_enable ? this->ps_desc->shininess : 0);
