...
/**
	...
	26-Feb-2003   Ilya    created
*/
void nGfxServer::makeModeString() {
	sprintf(this->disp_mode,"dev(%s)-type(%s)-w(%d)-h(%d)-bpp(%d)-zbuf(%d)-sbuf(%d)-samples(%d)",
		this->disp_dev,this->disp_type,this->disp_w,
		this->disp_h, this->disp_bpp, this->disp_zbufbits, 
		this->disp_sbufbits, this->disp_samples);
}
