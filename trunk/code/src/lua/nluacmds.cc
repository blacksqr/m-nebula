...
int luacmd_Mangle(lua_State* L)
{
    ...
    nFileServer2* fs = nLuaServer::kernelServer->GetFileServer2();
	stl_string path;    
    if (!fs->ManglePath(lua_tostring(L, -1), path))
    {
        n_message("Failed to mangle %s", path.c_str());
        lua_settop(L, 0);
        lua_pushnil(L);
        return 1;
    }
...
int luacmd_BeginCmds(lua_State* L)
{
    ...
    /*
      Class may not have been opened yet, so get the kernel the
      sort it out.
    */
    const char* className = lua_tostring(L, -2);
    nClass* clazz = nLuaServer::kernelServer->OpenClass(className);
    if (clazz)
        clazz->BeginScriptCmds(int(lua_tonumber(L, -1)));
    else
        n_error("Failed to open class %s!\n", className);
    /*
      Have to do this to keep the kernel server happy,
      lucky for us this won't actually unload the class
    */
    nLuaServer::kernelServer->CloseClass(clazz);
