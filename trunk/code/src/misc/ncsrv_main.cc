          ...
          : ref_is(ks,this), ref_gs(ks,this), ref_ss(ks,this)
{
    ...
    n_memset(this->input_buffer,0,sizeof(this->input_buffer));
    this->cursor_pos    = 0;
    this->overstrike    = false;
    this->ctrl_down     = false;
    this->scroll_offset = 0;
    this->act_hist      = 0;
    n_memset(this->watch_pattern,0,sizeof(this->watch_pattern));
}
...
bool nConServer::EditLine(nInputEvent *ie)
{
    ...
    if ((ie->GetType() == N_INPUT_KEY_CHAR) && (ie->GetChar() >= ' ')) {
        ...
        this->cursor_pos = insertChar((char)ie->GetChar(),
                                      this->input_buffer,
                                      sizeof(input_buffer),
                                      this->cursor_pos,
                                      overstrike);
            ...
            case N_KEY_RETURN:
                {
                    ...
                    scriptServer->Run(this->input_buffer, res);
