    ...
    if (cur_str) {
        ...
        if ( child ) {
            ...
            if (!cl) {				
                ...
                //if (die_on_error) n_error("Aborting.\n");
                //return NULL;
				// - removed by Ilya, to allow dynamic node replacement by new one descendant in the class hierarchy
				child->Remove();
                child->Release();
                child = 0;
...
*/
nKernelServer::nKernelServer(int _argc, const char** _argv)
             : class_list(128),
               toc_list(128),
               num_objects(0),
               quit_request(0),
...
*/
nRoot *nKernelServer::New(const char *cl_name, const char *path, bool ahead)
{
    n_assert(cl_name);
    n_assert(path);
    nRoot *o = _checkCreatePath(cl_name,path,true,ahead);
    n_assert(o);
    return o;
}
...
/**
  ...
  @param ahead Flag on true add this node ahead of the other chaild nodes. 

  - 24-Oct-99   floh    created
  - 04-Oct-00   floh    + keep pointer to last created object
  - 10-Jun-02   zeev    add the ahead param flag to enable adding the node ahead of all others.
*/
nRoot *nKernelServer::NewNoFail(const char *cl_name, const char *path, bool ahead)
{
    n_assert(cl_name);
    n_assert(path);
    nRoot *o = _checkCreatePath(cl_name,path,false,ahead);
    return o;
}
...
//--------------------------------------------------------------------
/**
  @brief Look up a path check if it's class instance 
  and return the corresponding nRoot object.

  @param path A path
  @return the nRoot object for the specified @c path.

  - 20-Jul-2006 ilya Kliot    created  
*/
nRoot *nKernelServer::LookupClass(const char *path, const char* classname) {
    if (path && classname) {
        nRoot* n = this->Lookup(path);
        if (n && n->IsA(this->FindClass(classname))) {
            return n;         
        } else {
            return 0;        
        }
...
nRoot *nKernelServer::SetParent(nRoot* thisObj,nRoot*toParent )
{
    n_assert(thisObj);
    n_assert(toParent);
    thisObj->Remove();
    toParent->AddTail(thisObj);
...
//--------------------------------------------------------------------
/**
  @brief Set the current working directory within the hierarchy to the
  given path.
  ...
  - 04-Oct-98   floh    char * -> const char *
*/
nRoot *nKernelServer::SetCwd(const char *path)
{
    nRoot *o = this->_lookupPath(path);
    if (o) 
		this->cwd = o;
...
//--------------------------------------------------------------------
/**
  @brief Check for node in cwd stack, before delete, of found replace by '/'.
  - 04-Mar-2008   Ilya & Rachel created
*/
void nKernelServer::ReplaceCwdInStack(nRoot* old_node)
{
	if (this->cwd_stack.IsEmpty()) 
		return;

	nRoot* o = this->cwd_stack.Pop();
	if (o == old_node)
	{		
		o = this->root;	
		n_printf("WARNING: Cwd stack was orphaned, setting cwd in stack to '/'\n");
	}
	else
		this->ReplaceCwdInStack(old_node);
	this->cwd_stack.Push(o);
}
...
//------------------------------------------------------------------------------
/**
    updates fileServer pointer to nFileServer2, allows to change file server to 
	descendant class.

    @param  pointer to fs, if NULL then trying to get file server by path
*/
void   nKernelServer::UpdateFileServer(nFileServer2* fs) 
{
	if (fs)
		this->fileServer = fs;
	else
		this->fileServer = (nFileServer2*)this->Lookup("/sys/servers/file2");

	n_assert(this->fileServer);
	if (!this->fileServer)
		n_error("FileServer Update failed. Cannot continue.\n");
