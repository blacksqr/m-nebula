...
static void n_removechildren(void *, nCmd *);
...
void nroot_initcmds(nClass *cl)
{
	...
	cl->AddCmd("v_removechildren_v",'RCHD', n_removechildren);
    cl->EndCmds();
}
...
{
    nRoot* self = (nRoot*)o;	  
    cmd->Out()->SetS(self->GetFullName().c_str());
}
...
/**
	...
	remove the children
*/
static void n_removechildren(void *o, nCmd*)
{
	nRoot* self = (nRoot*)o;
	self->RemoveChildren(); 
}
