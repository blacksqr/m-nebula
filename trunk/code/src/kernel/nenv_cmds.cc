...
void nenv_initcmds(nClass *cl)
{
    ...
    cl->AddCmd("u_getu_v",'GETU',n_getu);
    ...
    cl->AddCmd("v_setu_u",'SETU',n_setu);
...
static void n_gettype(void *o, nCmd *cmd)
{
    ...
    switch (t) 
	{
        ...
        case nArg::ARGTYPE_USTRING:st="ustring"; break;
...
/**
    ...
    @info
    Returns the content of the variable if it is a unicode string 
    variable. If it's not a unicode string variable "" is returned. 
*/
static void n_getu(void *o, nCmd *cmd)
{
    nEnv *self = (nEnv *) o;
    {
        const wchar *u = L"<error>";
        if (self->GetType() == nArg::ARGTYPE_USTRING) u = self->GetU();
        else n_printf("Not a Unicode string object!\n");
        cmd->Out()->SetU(u);
    }
}
...
/**
    ...
    @info
    Sets the content of the variable to the passed unicode string. 
*/
static void n_setu(void *o, nCmd *cmd)
{
    nEnv *self = (nEnv *) o;
    self->SetU(cmd->In()->GetU());
}
    ...
    if (nRoot::SaveCmds(fs)) {
        ...
        switch (this->GetType()) {
            ...
            case nArg::ARGTYPE_USTRING:
                if ((cmd = fs->GetCmd(this,'SETU'))) {
                    cmd->In()->SetU(this->GetU());
                    fs->PutCmd(cmd);
                }
                break;
            case nArg::ARGTYPE_BOOL:
                if ((cmd = fs->GetCmd(this,'SETB'))) {
                    cmd->In()->SetB(this->GetB());
                    fs->PutCmd(cmd);
                }
                break;
            case nArg::ARGTYPE_OBJECT:
                if ((cmd = fs->GetCmd(this,'SETO'))) {
