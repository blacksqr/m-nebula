...
void nBumpMeshNode::setupTangentSpace(nVertexBuffer* mesh, nIndexBuffer* tris)
{
  ...
  // init dot3info list
  dot3info.clear();
  dot3info.resize(mesh->GetNumVertices());
  for (i = 0; i < mesh->GetNumVertices(); i++)
    dot3info.push_back(nDot3Info(vector3(0,0,0),
                                vector3(0,0,0),
                                vector3(0,0,0)));
  ...
  // uvs.
  std::vector<nTempVertex> tempList;
  for (i = 0; i < mesh->GetNumVertices(); i++)
  {
    ...
    for (int j = i; j < mesh->GetNumVertices(); j++)
    {
      ...
      if (pos.fuz_eq(npos, 0.00001f))
      {
        nTempVertex* tv=NULL;
		int tnum = tempList.size();

        // see if that position is already in the list
        for (int klaus=0; klaus < tnum; klaus++)
        {
          if (tempList[klaus].p.fuz_eq(pos, 0.00001f))
          {
            tv = &tempList[klaus];
            break;
          }
        ...
        if (!tv)
        {
          tempList.push_back(nTempVertex());
          tv = &tempList[tempList.size()-1];
        }
        tv->p = pos;
        tv->indices.push_back(j);
      }
    }
  }
