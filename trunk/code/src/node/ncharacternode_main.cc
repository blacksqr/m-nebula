...
{
    n_assert(filename);
    this->animFilePath.Set(this->refFileServer.get(), filename);
...
nCharacterNode::Compute(nSceneGraph2* sceneGraph)
{
    ...
    if (this->renderSkeleton)
    {
        ...
        if (this->refPrimServer.isvalid()) {
            chr->RenderSkeleton(sceneGraph, this->refPrimServer.get(), this->skeletonColor);
        }
    }
