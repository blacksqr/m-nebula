               ...
               mouseInvert(false)
{
    ...
    this->act_script = 0;
    n_memset(this->script_array,0,sizeof(this->script_array));
...
/**
    ...
    @param  callbackProc    the script procedure to call, can be NULL if no
                            procedure call should happen
*/
void
nInputServer::Trigger(double /*time*/)
{
    ...
    {
        char n[N_MAXPATH];
        sprintf(n,"is_%s",is->GetName());
        nQuickWatch qw(kernelServer,n);
        sprintf(n,"%s %f",is->GetButton()?"true":"false",is->GetSlider());
        qw->SetS(n);
    }
}
