...
#	24-Jul-2007 Ilya	+ support for Visual Studio 2005
#	10-Oct-2007 Ilya	+ usage of tcl uuid
#
#--------------------------------------------------------------------
package require Tcl 8.2
package require uuid 

set release_preprocessor_defs "_NEBULA_DEVICE_;WIN32;__NEBULA_MEM_MANAGER__;NDEBUG;NT_PLUGIN;__WIN32__;_CRT_SECURE_NO_DEPRECATE;NO_STLPORT;__USE_SSE__;_WIN32_WINNT=0x0501;"
set debug_preprocessor_defs "_NEBULA_DEVICE_;WIN32;__NEBULA_MEM_MANAGER__;_DEBUG;NT_PLUGIN;__WIN32__;_CRT_SECURE_NO_DEPRECATE;NO_STLPORT;__USE_SSE__;_WIN32_WINNT=0x0501;"
set release_libpath "..\\..\\bin\\win32,..\\lib\\win32_vc_i386"
set debug_libpath "..\\..\\bin\\win32d,..\\lib\\win32_vc_i386d,..\\lib\\win32_vc_i386"
set incpath "..\\inc"
...
proc gen_package_vcproj {name} {
    ...
    puts $cid "	Name=\"$name\""
	puts $cid "	ProjectGUID=\"{$tar($i,uuid)}\""
	puts $cid "	>"
    puts $cid "	<Platforms>"
    puts $cid "		<Platform"
    puts $cid "			Name=\"Win32\""
	...
	puts $cid "			/>"
	puts $cid "			<Tool"
	puts $cid "				Name=\"VCXMLDataGeneratorTool\""
	puts $cid "			/>"
	puts $cid "			<Tool"
	...
	puts $cid "				DisableSpecificWarnings=\"4706;4127;4530\""
	puts $cid "			/>"
    puts $cid "			<Tool"
    puts $cid "				Name=\"VCManagedResourceCompilerTool\""
	puts $cid "			/>"
	puts $cid "			<Tool"
	...
	puts $cid "			/>"
    puts $cid "			<Tool"
    puts $cid "				Name=\"VCALinkTool\""
    puts $cid "			/>"
    puts $cid "			<Tool"
    puts $cid "				Name=\"VCManifestTool\""
    puts $cid "			/>"	
    puts $cid "			<Tool"	
    puts $cid "				Name=\"VCXDCMakeTool\""
	puts $cid "			/>"
    puts $cid "			<Tool"	
    puts $cid "				Name=\"VCBscMakeTool\""
	puts $cid "			/>"
    puts $cid "			<Tool"	
    puts $cid "				Name=\"VCFxCopTool\""
	puts $cid "			/>"	
    puts $cid "			<Tool"	
    puts $cid "				Name=\"VCAppVerifierTool\""
	puts $cid "			/>"	
    puts $cid "			<Tool"	
...
proc gen_solution {name list_of_targets} {
	...
	#generate uuid for all projects
    ...
    for {set i 0} {$i < [llength $list_of_targets]} {incr i} {
        ...
        if {(($tar($t,platform)=="all") || ($tar($t,platform)=="win32"))} {
		    ...
		    set tar($t,uuid) [string toupper [uuid::uuid generate]]
			...
			if {[llength $tar($t,depends)] > 0} {
	            ...
	            for {set k 0} {$k < [llength $tar($t,depends)]} {incr k} {
	                ...
	                if {(($tar($d,platform)=="win32") || ($tar($d,platform)=="all"))} {
						...
						puts $cid "		{$tar($d,uuid)} = {$tar($d,uuid)}"
	                    incr depcount
	                }
	            }			
    ...
    for {set i 0} {$i < [llength $list_of_targets]} {incr i} {
        ...
        # ignore any non-win32-packages
        if {(($tar($t,platform)=="all") || ($tar($t,platform)=="win32"))} {
            puts $cid "		{$tar($t,uuid)}.Debug|Win32.ActiveCfg = Debug|Win32"
            puts $cid "		{$tar($t,uuid)}.Debug|Win32.Build.0 = Debug|Win32"
            puts $cid "		{$tar($t,uuid)}.Release|Win32.ActiveCfg = Release|Win32"
            puts $cid "		{$tar($t,uuid)}.Release|Win32.Build.0 = Release|Win32"
        }
    }
